...
from .postprocess import yolo3_decode
...
def softmax_focal_loss(y_true, y_pred, gamma=2.0, alpha=0.25):
    ...
    # Arguments
        ...
        alpha: optional alpha weighting factor to balance positives vs
            negatives.
    # Returns
...
def box_giou(b_true, b_pred):
    ...
    b_pred: predict boxes tensor, shape=(batch, feat_w, feat_h, anchor_num, 4),
        xywh
    Returns
    -------
...
def yolo3_loss(
        args, anchors, num_classes, ignore_thresh=.5, label_smoothing=0,
        elim_grid_sense=False, use_focal_loss=False, use_focal_obj_loss=False,
    ...
    grid_shapes = [
        K.cast(K.shape(yolo_outputs[i])[1: 3],
               K.dtype(y_true[0])) for i in range(num_layers)]
    loss = 0
    ...
    for i in range(num_layers):
        ...
        if label_smoothing:
            ...
            true_objectness_probs = _smooth_labels(
        ...
        raw_true_wh = K.log(
            y_true[i][..., 2: 4] / anchors[anchor_mask[i]] *
            input_shape[:: -1])
        ...
        _, ignore_mask = tf.while_loop(
            lambda b, *args: b < batch_size, loop_body, [0, ignore_mask])
