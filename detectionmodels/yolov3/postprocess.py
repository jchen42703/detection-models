...
def yolo3_decode(
        feats, anchors, num_classes, input_shape, scale_x_y=None,
        calc_loss=False):
    ...
    if scale_x_y:
        ...
        box_xy = (
            box_xy_tmp + grid) / K.cast(grid_shape[..., ::-1], K.dtype(feats))
    else:
        box_xy = (K.sigmoid(
            feats[..., :2]) + grid) / K.cast(grid_shape[..., ::-1],
                                             K.dtype(feats))
    box_wh = K.exp(
        feats[..., 2:4]) * anchors_tensor / K.cast(input_shape[..., ::-1],
                                                   K.dtype(feats))
...
def yolo3_boxes_and_scores(
        feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs = yolo3_decode(
...
def get_anchorset(anchors, num_layers, layer_idx):
...
def batched_yolo3_postprocess(args,
    ...
    box_scores = []
    for layer_idx in range(num_layers):
        _boxes, _box_scores = batched_yolo3_boxes_and_scores(
            yolo_outputs[layer_idx],
            anchors[anchor_mask[layer_idx]],
            num_classes, input_shape, image_shape,
            scale_x_y=scale_x_y[layer_idx])
...
def batched_yolo3_prenms(args,
    ...
    for layer_idx in range(num_layers):
        ...
        # Adjust preditions to each spatial grid point and anchor size.
        box_xy = (K.sigmoid(
            reshape_feats[..., :2]) + grid) / K.cast(grid_shape[::-1],
                                                     K.dtype(reshape_feats))
        box_wh = K.exp(
            reshape_feats[..., 2: 4]) * anchors_tensor / K.cast(
            input_shape[:: -1],
            K.dtype(reshape_feats))
        ...
        # new_shape = K.round(image_shape * K.min(input_shape/image_shape))
        new_shape = K.cast(
            image_shape * K.min(input_shape / image_shape),
            dtype='int32')
        ...
        _box_scores = K.reshape(
            _box_scores, [-1, total_anchor_num, num_classes])
...
class Yolo3PostProcessLayer(Layer):
    ...
    def call(self, x):
        ...
        for layer_idx in range(self.num_layers):
            ...
            reshape_feats = K.reshape(
                ...
                [-1, grid_shape[0],
                 grid_shape[1],
                 num_anchors, self.num_classes + 5])
            ...
            _box_scores = box_confidence * box_class_probs
            _box_scores = K.reshape(
                _box_scores, [-1, total_anchor_num, self.num_classes])
    ...
    def compute_output_shape(self, input_shape):
        ...
        return [(shape_yolo[0], self.total_anchor_num, 4),
                (shape_image[0], self.total_anchor_num, self.num_classes)]
