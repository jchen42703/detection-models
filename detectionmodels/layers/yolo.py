...
from tensorflow.keras.layers import Concatenate, Lambda, BatchNormalization, \
    MaxPooling2D
from .general import YoloConv2D, YoloDepthwiseConv2D
...
def compose(*funcs):
    ...
    Used to compose layers, analogous to tf.keras.Sequential
    """
...
def DarknetConv2D(*args, **kwargs):
    ...
    This layer is a basic convolution with L2 Regularization and
    padding="same".
    """
...
def DarknetDepthwiseConv2D(*args, **kwargs):
    ...
    This layer is a basic convolutional layer with L2 Regularization and
    the padding changes to valid if strides == (2, 2)  or 'same' otherwise.
    """
...
def Darknet_Depthwise_Separable_Conv2D_BN_Leaky(
    ...
    return compose(
        DarknetDepthwiseConv2D(
            kernel_size, name='conv_dw_' + block_id_str, **
            no_bias_kwargs),
        BatchNormalization(
            name='conv_dw_%s_bn' % block_id_str),
        LeakyReLU(
            alpha=0.1, name='conv_dw_%s_leaky_relu' % block_id_str),
        YoloConv2D(
            filters, (1, 1),
            padding='same', use_bias=False, strides=(1, 1),
            name='conv_pw_%s' % block_id_str),
        BatchNormalization(
            name='conv_pw_%s_bn' % block_id_str),
        LeakyReLU(
            alpha=0.1, name='conv_pw_%s_leaky_relu' % block_id_str))


def Depthwise_Separable_Conv2D_BN_Leaky(
        filters, kernel_size=(3, 3),
        block_id_str=None):
    """Depthwise Separable Convolution2D."""
    if not block_id_str:
    ...
    return compose(
        YoloDepthwiseConv2D(
            kernel_size, padding='same', name='conv_dw_' +
            block_id_str),
        BatchNormalization(
            name='conv_dw_%s_bn' % block_id_str),
        LeakyReLU(
...
def yolo2lite_predictions(
    ...
    x1 = compose(
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=f1_channel_num, kernel_size=(3, 3),
            block_id_str='pred_1'),
        Depthwise_Separable_Conv2D_BN_Leaky(
            filters=f1_channel_num, kernel_size=(3, 3),
            block_id_str='pred_2'))(f1)
    ...
    x = Concatenate()([x2_reshaped, x1])
    x = Depthwise_Separable_Conv2D_BN_Leaky(
        filters=f1_channel_num, kernel_size=(3, 3),
        block_id_str='pred_3')(x)
    y = DarknetConv2D(
        num_anchors * (num_classes + 5),
        (1, 1),
        name='predict_conv')(x)
